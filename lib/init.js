...
exports.default = function (args) {
  ...
  var absoluteTarget = _path2.default.resolve(process.cwd(), target);
  var tmpTarget = _path2.default.resolve(process.cwd() + '/' + target + '/../doxity-tmp-' + Date.now());
  _fs2.default.mkdirSync(tmpTarget);
  ...
  (0, _helpers.clearDirectory)(absoluteTarget).then(function () {
    ...
    return new Promise(function (resolve) {
      _request2.default.get(source).pipe(_tar2.default.x({ cwd: tmpTarget })).on('finish', resolve);
    });
  })
  ...
  .then(function () {
    ...
    npmInstall.on('close', function () {
      ...
      var doxityrcFile = process.cwd() + '/' + _constants.DOXITYRC_FILE;
