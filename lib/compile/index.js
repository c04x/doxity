...
exports.default = function (opts) {
  var output = process.cwd() + '/' + opts.target + '/' + opts.dir;
...
function compile(_ref) {
  ...
  var pkgConfig = {};
  try {
    pkgConfig = JSON.parse(_fs2.default.readFileSync(process.cwd() + '/package.json'));
  } catch (e) {
  ...
  var configFile = process.cwd() + '/' + target + '/' + _constants.CONFIG_FILE;
  ...
  try {
    ...
    config = _extends({}, config, JSON.parse(_fs2.default.readFileSync(process.cwd() + '/' + _constants.DOXITYRC_FILE).toString()));
  } catch (e) {} /* do nothing */
  ...
  // copy the readme
  try {
    var readmeFile = _glob2.default.sync(process.cwd() + '/' + _constants.README_FILE, { nocase: true })[0];
    var readmeTarget = process.cwd() + '/' + target + '/' + _constants.README_TARGET;
